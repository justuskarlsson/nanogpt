...
def lm_head_fp8(x: Tensor, w: Tensor) -> Tensor:
    ...
    out: Tensor = torch.ops.nanogpt.mm(
        _x, w, x_s=2.0, w_s=32.0, grad_s=2.0**29
    )[0]
...
class Block(nn.Module):
    def __init__(self, model_dim: int, num_heads: int, layer_idx: int):
        ...
        self.attn = (
            CausalSelfAttention(model_dim, num_heads)
            if layer_idx != 7
            else None
        )
...
class GPT(nn.Module):
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [
                Block(model_dim, num_heads, layer_idx)
    ...
    def forward(
        ...
        x = norm(x)
        logits = (
            lm_head_fp8(x, self.lm_head.weight)
            if self.training
            else self.lm_head(x)
        )
...
def distributed_data_generator(
    ...
    file_iter = iter(
        files
    )  # use cycle(files) if you want to do multi-epoch training
