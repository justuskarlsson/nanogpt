            ...
            def update_prev():
                ...
                for p_world, g_world in zip(params_world, update_buffers):
                    p_world.data.add_(
                        ...
                        alpha=-lr
                        * max(1, p_world.size(0) / p_world.size(1)) ** 0.5,
                    )
...
class Block(nn.Module):
    ...
    def __init__(self, model_dim, num_heads, use_attn=True):
        super().__init__()
        self.attn = (
            CausalSelfAttention(model_dim, num_heads) if use_attn else None
        )
...
class GPT(nn.Module):
    ...
    def __init__(self, vocab_size, num_layers, num_heads, model_dim):
        ...
        self.blocks = nn.ModuleList(
            [
                Block(model_dim, num_heads, use_attn=(i != 7))
    ...
    def forward(self, inputs, targets, sliding_window_num_blocks):
        ...
        def create_doc_swc_block_mask(sliding_window_num_blocks):
            ...
            window_full_bm = (
                window_bm  # block-wise sliding window by @YouJiacheng
            )
        ...
        x0 = norm(
            self.embed(inputs[None]).bfloat16()
        ...
        logits = 15 * torch.tanh(
            logits / 15
        )  # @Grad62304977 added tanh softcapping, @KoszarskyB reduced it from 30 to 15
        logits = logits.float()
...
class DistributedDataLoader:
    ...
    def next_batch(self, batch_size):
        ...
        inputs = device_batch_tokens[:-1].to(
            device="cuda", dtype=torch.int32, non_blocking=True
        )
        targets = device_batch_tokens[1:].to(
...
for step in range(train_steps + 1):
    ...
    if last_step or (
        ...
        for _ in range(val_steps):
            with torch.no_grad():
                ...
                val_loss += ddp_model(
                    inputs_val, targets_val, sliding_window_num_blocks
                )
    ...
    for micro_inputs_train, micro_targets_train in zip(
        inputs_train.split(micro_bs), targets_train.split(micro_bs)
    ):
        ddp_model(
            micro_inputs_train, micro_targets_train, sliding_window_num_blocks
        ).backward()
